...
`ENROLLMENT_SECRET` is mounted into the container as a volume, not an env var (see `config/manager/manager.yaml`'s `enrollment-secret` volume — this operator never needs to know this Secret's own object name, only that fixed mount path), and must be readable at startup or the container fails fast (`cmd/main.go`'s `setupConvexIntegration`). After first registration the operator persists its issued token (see `internal/tokenstore`) and reuses it across restarts — but it also keeps re-reading that mounted file in the background (`internal/convexclient`'s `EnrollmentSecretWatcher`), so if you rotate `ENROLLMENT_SECRET` later (e.g. `kubectl edit secret ai-cloud-operator-env -n ai-cloud-operator-system`), the operator picks up the new value and re-registers on its own — no `rollout restart` needed for a rotation, only for this initial creation. Expect it within a couple of minutes: kubelet only refreshes a mounted Secret volume's content on its own periodic sync (about a minute), on top of which the operator's own heartbeat interval has to tick over before it re-checks the file.

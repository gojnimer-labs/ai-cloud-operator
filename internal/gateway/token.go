...
// Package gateway lets Convex mint short-lived, per-workload access tokens
// that end users' browsers present directly to the operator's /gw/* routes,
// which the operator verifies locally (no callback to Convex per request) and
// then reverse-proxies into the cluster via the Kubernetes API server's
// services/proxy subresource — the same mechanism kubectl proxy/the
// Dashboard use, so the operator never needs direct pod-network reachability.
package gateway
...
// namespace/name specifically — a token minted for one workload must not be
...
// the base64url payload string, keyed by secret). Mirrors
// ai-cloud-v2/convex/gateway/token.ts's mintGatewayToken exactly — the two
// sides only need to agree on this byte layout.

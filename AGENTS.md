# ai-cloud-operator - AI Agent Guide
...
In this project markers aren't confined to the controller file — they're
co-located with whatever code declares the verbs (`internal/tokenstore`,
`internal/gateway/keystore.go`, `internal/gateway/proxy.go`,
`internal/podexec`, `internal/convexclient/enrollment.go`). Before adding a
new one, `grep -rn "kubebuilder:rbac" internal/` to see the full set —
`config/rbac/role.yaml` (regenerated by `make manifests`) aggregates all of
them regardless of which file they live in.
